...
| 1.2 | `public/service-mappings.json` import warning on `tauri:dev` boot | Pre-existing; surfaced in 07C.0 review | low | `closed` — 07I.a.1 removed the `public/` JSON import and uses explicit `fetch('/service-mappings.json')`; see commit `fix(phase-07i): ckpt 07I.a.1; service-mappings.json load without vite warning` | Enrique + Codex | closed |
...
| 1.7 | Tauri `default.json` capabilities allowlist still uses starting patterns from 07B.1; 07B.4 audit was light-touch. Full audit needed before ship | 07B.4 self-assessment | medium | `closed` — 07I.a.2 audited HTTP/fs/dialog scopes, pruned unused entries, added Datadog EU coverage, and wrote `docs/testing/capabilities-audit-2026-04.md`; see commit `feat(phase-07i): ckpt 07I.a.2; capabilities allowlist audit + prune` | Codex | closed |
...
| 1.11 | **Automated smoke testing limited to Windows + Linux.** Tauri v2 WebDriver (tauri-driver + WebdriverIO) does not support macOS — WKWebView does not expose the WebDriver protocol. macOS coverage stays on manual runbook walks per `docs/testing/runbooks/*.md`; Windows + Linux get `/smoke-tauri <name>` automation via 07J.3 harness | 07J.3 pre-flight 2026-04-23 (Codex halt surfaced Tauri v2 WebDriver reality — `tauri-driver` is a Cargo binary, no npm path, no macOS support) | low | `accepted` — closed-distribution Mac recipients are a small set; manual runbook walks remain the macOS QA path. Phase 08: revisit if Tauri v2 exposes a macOS WebDriver path or if coverage need justifies a custom AppKit-side automation shim | Enrique | accepted (Phase 08 revisit) |
| 1.12 | **Windows msedgedriver prerequisite for `/smoke-tauri`.** `tauri-driver` on Windows proxies to Microsoft Edge WebDriver (`msedgedriver.exe`), which must be (a) installed and on PATH, or (b) pointed at via the `TAURI_NATIVE_DRIVER_PATH` env var. Version must match the installed Edge browser. Not needed for the app itself — only for automated smoke runs | 07J.3.b Batch D gate — `/smoke-tauri splash` exited 1 because tauri-driver couldn't start msedgedriver | low | `accepted` — environmental dev-setup, documented in `tools/claude-automation/README.md`. Same class as §1.1 (cargo PATH) and §1.9 (sandbox EPERM). User installs `msedgedriver.exe` matching their Edge version (download from Microsoft's EdgeDriver page) once per machine | Enrique | accepted |
...
| 4.3 | **Installer format.** `bundle.targets: "all"` in `tauri.conf.json` produces both MSI + NSIS on Windows. Shipping both to closed recipients adds confusion | `src-tauri/tauri.conf.json:30` | `closed` — 07I.a.3 sets explicit `bundle.targets: ["nsis", "dmg"]`; see commit `feat(phase-07i): ckpt 07I.a.3; NSIS-only bundle + runtime app version` | Codex | closed |
...
| 4.6 | **App version baseline.** `tauri.conf.json` declares `version: "2.0.0"`; SplashScreen displays `v4.2.0` in its right slot (carried over from pre-Phase-07 auth chrome) | Conflict between canonical app version and UI chrome | `closed` — 07I.a.3 adds `useAppVersion()` with Tauri `getVersion()` runtime lookup and `package.json` fallback, then renders the splash right-slot version dynamically; see commit `feat(phase-07i): ckpt 07I.a.3; NSIS-only bundle + runtime app version` | Codex | closed |
...
| 5.9 | **Lint cleanup — Bucket B.** 56 remaining `@typescript-eslint/no-explicit-any` errors in source files (`LogContext.tsx`, `AIContext.tsx`, `unleashService.ts`, `logContextBuilder.ts`, `redactor.ts`, `anime.ts`, `types/event.ts`, `AIButton.tsx`, `AiPanel.tsx`, `ZendeskPanel.tsx`, `CorrelationSidebar.tsx`) + 4 `react-hooks/exhaustive-deps` warnings + 7 `react-refresh/only-export-components` errors. Pre-existing drift, not Phase 07 damage. Bucket A (config tuning + mechanical regex/prefer-const fixes) cleared in 07J.3 hygiene slice; remaining errors need per-site typing decisions. `server/` and `public/` directories ignored via ESLint config — dead pre-Tauri backend (`server/`) and static assets (`public/`) | 07I.a pre-flight lint baseline | Phase 08 |
