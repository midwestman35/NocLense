...
const PROVIDER_HOWTOS: Partial<Record<AIProviderId, { steps: string[]; linkText: string }>> = {
  gemini: {
    steps: [
  ...
  codex: {
    steps: [
      ...
      'Run codex login to authenticate (or set CODEX_API_KEY)',
      'Select Codex CLI in AI Settings — key field is optional',
    ],
    linkText: 'Codex CLI docs',
  },
};
...
export default function OnboardingPage({ onOpenSettings, onComplete, onSkip }: OnboardingPageProps) {
  ...
  return (
    <div className="flex flex-col h-screen w-full bg-[var(--bg-light)] text-[var(--text-primary)]">
      {/* Progress */}
        ...
        <div className="max-w-xl w-full">
          ...
          {step === 1 && (
            <div className="flex flex-col items-center text-center">
              ...
              <p className="text-[var(--text-secondary)] mb-6 leading-relaxed">
                The AI assistant helps you analyze SIP and telecom logs with natural-language
                questions, call-flow insights, and error summaries. Set it up in a few steps,
                or skip and configure later from the sidebar.
              </p>
              <ul className="text-left w-full space-y-2 text-sm text-[var(--text-secondary)] mb-8">
                ...
                <li className="flex items-center gap-2">
                  <ChevronRight size={16} className="text-[var(--accent-blue)] flex-shrink-0" />
                  Use free tiers (Gemini) or paid providers (Claude, Codex)
                </li>
              </ul>
            </div>
          )}
          ...
          {step === 2 && (
            <div>
              ...
              <p className="text-sm text-[var(--text-secondary)] mb-4">
                Pick one to get started. You can change it anytime in AI Settings.
              </p>
              <div className="space-y-2">
                {Object.values(AI_PROVIDERS).map((p) => (
                  <a
                    ...
                    className="block p-3 rounded-lg border border-[var(--border-color)] hover:border-[var(--accent-blue)]/50 hover:bg-[var(--accent-blue)]/5 transition-colors group"
                  >
                    <div className="flex items-center justify-between">
                      <span className="font-medium text-sm">{p.name}</span>
                      <ExternalLink size={14} className="text-[var(--text-secondary)] group-hover:text-[var(--accent-blue)]" />
                    </div>
                    <span className="text-xs text-[var(--text-secondary)]">{p.keyLabel}</span>
                  </a>
                ))}
              </div>
            </div>
          ...
          {step === 3 && (
            <div>
              ...
              <p className="text-sm text-[var(--text-secondary)] mb-4">
                Each provider has a free or trial tier. Follow the steps for your chosen provider.
              </p>
              <div className="space-y-4">
                {Object.entries(PROVIDER_HOWTOS).map(([id, howto]) => {
            ...
            {step === 4 && (
              <>
                ...
                <button
                  ...
                  onClick={onComplete}
                  disabled={!isReady}
                  className="w-full px-4 py-3 rounded-lg border border-[var(--border-color)] bg-[var(--card-bg)] text-[var(--text-primary)] font-medium flex items-center justify-center gap-2 hover:bg-[var(--bg-light)] disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                >
                  Continue to app
                  <ArrowRight size={18} />
                </button>
                <button
                  type="button"
                  onClick={onSkip}
                  className="w-full px-4 py-2.5 text-sm text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors"
                >
                  Skip for now
                </button>
              </>
            ...
            {step < 4 && (
              <>
                <div className="flex gap-3">
                  {step > 1 && (
                    <button
                      ...
                      className="px-4 py-2.5 rounded-lg border border-[var(--border-color)] bg-[var(--card-bg)] text-[var(--text-primary)] font-medium flex items-center gap-2 hover:bg-[var(--bg-light)] transition-colors"
                    >
                      <ArrowLeft size={18} />
                      Back
                    </button>
                ...
                <button
                  type="button"
                  onClick={onSkip}
                  className="w-full px-4 py-2.5 text-sm text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors mt-2"
                >
                  Skip for now
                </button>
              </>
