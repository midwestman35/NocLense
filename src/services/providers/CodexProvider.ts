...
export class CodexProvider implements LLMProvider {
  ...
  public initialize(apiKey: string, model: string = 'gpt-4.1-mini'): void {
  ...
  private ensureInitialized(): void {
    if (!this.apiKey) {
      throw new InvalidApiKeyError('Codex provider not initialized');
    }
  }
    ...
    try {
      const response = await fetch('https://api.openai.com/v1/responses', {
        ...
        body: JSON.stringify({
          ...
          input: prompt,
          max_output_tokens: Math.min(options?.maxTokens ?? 2048, 4096),
      ...
      throw new NetworkError('Codex request failed');
    }
  }
