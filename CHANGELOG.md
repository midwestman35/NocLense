...
### Documentation & Repository Cleanup (Gemini)
- **Consolidated Documentation**: Replaced legacy `HANDOFF.md` and scattered architecture notes with `CLAUDE.md`, `AGENTS.md`, and `docs/DEVELOPER_HANDOFF.md`.
- **Archived Stale Artifacts**: Moved legacy plans, session notes, and outdated runbooks to `docs/archive/`.
- **Purged Cross-Project Pollution**: Removed leftover templates from `BaseballManager` and `NocSight`.
- **Standardized Skills**: Consolidated agent skills into `.claude/skills/` and removed duplicate `.agents/skills/`.
- **Worktree Cleanup**: Removed orphaned git worktrees to reclaim disk space.
- **Improved .gitignore**: Added patterns for `.tmp-*/`, `.agents/`, and brainstorming logs.
