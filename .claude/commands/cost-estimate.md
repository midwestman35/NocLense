...
- Total lines of code (TypeScript, TSX, CSS, config files)
- Architectural complexity (frameworks, integrations, APIs)
- Advanced features (virtualized rendering, IndexedDB streaming, AI integrations, Electron IPC)
- Testing coverage
...
- Build and config files (`package.json`, `vite.config.ts`, `tsconfig*.json`)
...
- Simple UI components / CRUD: 30-50 lines/hour
- Complex React components (virtualized lists, context): 20-30 lines/hour
- State management / context providers: 20-30 lines/hour
- AI/LLM integration and provider abstraction: 10-20 lines/hour
- IndexedDB / large-file streaming: 10-20 lines/hour
- Electron IPC / main process: 15-25 lines/hour
- Parser / data transformation logic: 15-25 lines/hour
...
- Learning curve (new frameworks/APIs): +10-20% for specialized tech

**Calculate total hours** considering:
1. Base coding hours (lines of code / productivity rate by file type)
2. Multipliers for complexity and overhead
3. Specialized knowledge required (Electron security model, AI provider abstraction, virtual rendering)

## Step 3: Research Market Rates

Use WebSearch to find current 2026 hourly rates for:
- Senior full-stack TypeScript/React developers (5-10 years experience)
- Electron desktop application developers
- Contractors vs. employees
- Geographic variations (US markets: SF Bay Area, NYC, Austin, Remote)

Search queries to use:
- "senior full stack developer hourly rate 2026"
- "React TypeScript developer contractor rate 2026"
- "Electron desktop developer freelance rate 2026"
- "senior software engineer hourly rate United States 2026"
...
**Codebase Version**: [From CLAUDE.md or package.json version]

### Codebase Metrics

- **Total Lines of Code**: [number]
  - TypeScript (.ts): [number] lines
  - React/TSX (.tsx): [number] lines
  - CSS/Styles: [number] lines
  - Tests: [number] lines
  - Config & Other: [number] lines

- **Complexity Factors**:
  - Advanced frameworks: [list key ones]
  - Performance engineering: [virtualized rendering, IndexedDB streaming, etc.]
  - AI/LLM integrations: [providers, context building, rate limiting]
  - Desktop/Electron specifics: [IPC, safeStorage, preload bridge]
  - Third-party integrations: [APIs, services]

### Development Time Estimate

**Base Development Hours**: [number] hours
- Core UI Components: [hours] hours
- State Management & Contexts: [hours] hours
- AI/LLM Integration: [hours] hours
- Parser & Data Pipeline: [hours] hours
- Electron Layer: [hours] hours
- Testing: [hours] hours
...
**Senior Full-Stack Developer Rates (2026)**:
...
*Rationale*: [Explain which specializations drive the rate — e.g., Electron expertise, AI integration, high-performance rendering]
...
1. Rates based on US market averages (2026)
2. Full-time equivalent allocation for all roles
3. Includes complete implementation of all shipped features
4. Does not include:
   - Marketing & sales
   - Legal & compliance
   - Office/equipment
...
Use `find . -name "*.ts" -o -name "*.tsx" | xargs stat` to get file mod times. Apply same session clustering logic.
